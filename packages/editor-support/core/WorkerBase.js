...
class WorkerBase {
    ...
    _updateProgress(step) {
        this._progress += step;
        Editor.Ipc.sendToAll('elestorm-uieditor:state-changed',
    ...
    _execTime(tag) {
        ...
        if (this._debug) {
            ...
            Editor.log('[UIEditor] [' + tag + '] ' + times.toString() + 's');
        }
        this._time = current;
    }
}
...
function registerWorker(workerClass, runEvent) {
    Electron.ipcRenderer.on('elestorm-uieditor:' + runEvent, (event, state, opts) => {
